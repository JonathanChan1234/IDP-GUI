    ...
    if spotSelected == 0:
        if illegalOccupied1 == 0:
            if counting1 == 0 and selectTime1 != 0:
                ...
                #qrc = str(code1)
                #img = qrcode.make(qrc)
                #displayButton.config(image = img)
...
def clearPos():
    ...
    if beginning == 1:
        ...
        if remaining2 != 0 and occupied2 == 0:
            ...
            beginning = 0
            firstClear = 0
    displaytime1.after(5000,clearPos)
...
displayButton = tk.Button(win, height=5, width=15)
displayButton.place(relx=0.3,rely=0.8, anchor=tk.CENTER)
#displayButton.config(img=qrcode.make("123"))
